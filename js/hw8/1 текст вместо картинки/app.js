...
function handleClick(clickedButtonEvent) {
    ...
    let descriptionTxt = card.button.innerText;

    if (descriptionTxt == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (descriptionTxt == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let txtImg = "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Mollitia, itaque error nam vero incidunt officia harum suscipit culpa voluptatibus sed doloribus, quis voluptatem, dignissimos ducimus architecto ipsam! Ab, cum praesentium!";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML("afterend", `<div class="desc">${txtImg}</div>`)
